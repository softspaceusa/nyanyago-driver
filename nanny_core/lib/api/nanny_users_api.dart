...
class NannyUsersApi {
  ...
  static Future<ApiResponse<void>> updateMe(UpdateMeRequest request) async {
    return RequestBuilder().create(
      dioRequest: DioRequest.dio.put("/users/update_me", data: request.toJson()),
    );
  }
  ...
  static Future<ApiResponse<int>> addDebitCard(AddDebitCardRequest request) async {
    return RequestBuilder<int>().create(
      dioRequest: DioRequest.dio.post("/users/add_debit_card", data: request.toJson()),
  ...
  static Future<ApiResponse<PaymentInitData>> startPayment(StartPaymentRequest request) async {
    return RequestBuilder<PaymentInitData>().create(
      dioRequest: DioRequest.dio.post("/users/start_payment", data: request.toJson()),
      onSuccess: (response) => PaymentInitData.fromJson(response.data),
    );
  }

  static Future<ApiResponse<SbpInitData>> startSbpPayment(StartSbpPaymentRequest request) async {
    return RequestBuilder<SbpInitData>().create(
      dioRequest: DioRequest.dio.post("/users/start_sbp_payment", data: request.toJson()),
      onSuccess: (response) => SbpInitData.fromJson(response.data['payment']),
    );
  }

  static Future<ApiResponse<Check3DsData>> confirmPayment(ConfirmPaymentRequest request) async {
    return RequestBuilder<Check3DsData>().create(
      dioRequest: DioRequest.dio.post("/users/confirm_payment", data: request.toJson()),
  ...
  static Future<ApiResponse<void>> addMoney(AddMoneyRequest request) async {
    return RequestBuilder().create(
      dioRequest: DioRequest.dio.post("/users/add_money", data: request.toJson()),
      errorCodeMsgs: {
        402: "Не удалось пополнить баланс!\nЕсли деньги с вашей карты снялись, то обратитесь в поддержку."
      }
    );
  }
