...
class NannyFranchiseApi {
  static Future<ApiResponse<List<UserInfo<void>>>> getDrivers(
      {Map<String, dynamic>? queryParameters}) async {
    return RequestBuilder<List<UserInfo<void>>>().create(
      dioRequest: DioRequest.dio
          .get("/franchises/drivers", queryParameters: queryParameters),
      onSuccess: (response) => List<UserInfo>.from(
        response.data['drivers'].map(
          (x) => UserInfo.fromJson(x),
        ),
      ),
    );
  }
  ...
  static Future<ApiResponse<List<UserInfo<void>>>> getNewDrivers() async {
    return RequestBuilder<List<UserInfo<void>>>().create(
      ...
      onSuccess: (response) => List<UserInfo>.from(
          response.data['drivers'].map((x) => UserInfo.fromJson(x))),
    );
  }

  static Future<ApiResponse<List<PayoutModel>>> getPartnerPayouts() async {
    return RequestBuilder<List<PayoutModel>>().create(
      dioRequest: DioRequest.dio.get("/franchises/partner_payouts"),
      onSuccess: (response) => List<PayoutModel>.from(
          response.data['payouts'].map((x) => PayoutModel.fromJson(x))),
    );
  }
  ...
  static Future<ApiResponse<List<UserInfo>>> agreePaymentRequest({
    ...
    return RequestBuilder<List<UserInfo>>().create(
      dioRequest:
          DioRequest.dio.post("/franchises/agree_payment_request", data: data),
    );
  }
  ...
  static Future<ApiResponse<List<PaymentRequests>>> getPaymentRequests() {
    return RequestBuilder<List<PaymentRequests>>().create(
      ...
      onSuccess: (response) =>
          response.data['result'].map((x) => PaymentRequests.fromJson(x)),
    );
  }
  ...
  static Future<ApiResponse<void>> createUser(CreateUserRequest request) async {
    return RequestBuilder<void>().create(
      dioRequest:
          DioRequest.dio.post("/franchises/new_user", data: request.toJson()),
    );
  }

  static Future<ApiResponse<void>> addBonusMoney(
      {required int id, required int amount}) {
    var data = <String, dynamic>{"id_driver": id, "amount": amount};

    return RequestBuilder<void>().create(
        dioRequest:
            DioRequest.dio.post("/franchises/add_bonus_money", data: data),
        errorCodeMsgs: {404: "Водитель не найден"});
  }

  static Future<ApiResponse<void>> addFinemoney(
      {required int id, required int amount}) {
    var data = <String, dynamic>{"id_driver": id, "amount": amount};

    return RequestBuilder<void>().create(
        dioRequest:
            DioRequest.dio.post("/franchises/add_fine_money", data: data),
        errorCodeMsgs: {404: "Водитель не найден"});
  }
  ...
  static Future<ApiResponse<void>> createTariff(FranchiseTariff request) {
    return RequestBuilder<void>().create(
        dioRequest:
            DioRequest.dio.post("/franchises/tariff", data: request.toJson()));
  }

  static Future<ApiResponse<void>> deleteTariff(int id) {
    return RequestBuilder<void>()
        .create(dioRequest: DioRequest.dio.delete("/franchises/tariff?id=$id"));
  }

  static Future<ApiResponse<void>> updateTariff(FranchiseTariff data) {
    return RequestBuilder<void>().create(
        dioRequest:
            DioRequest.dio.delete("/franchises/tariff", data: data.toJson()));
  }

  static Future<ApiResponse<void>> responseNewDriver(
      ResponseNewDriverRequest request) {
    return RequestBuilder<void>().create(
        dioRequest: DioRequest.dio
            .post("/franchises/response_new_driver", data: request.toJson()),
        errorCodeMsgs: {201: "Пользователь не найден!"});
  }
}
