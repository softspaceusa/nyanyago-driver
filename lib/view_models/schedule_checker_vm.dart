...
class ScheduleCheckerVm extends ViewModelBase {
  ScheduleCheckerVm({
    ...
    request = WantScheduleRequest()
      ..idSchedule = schedule.id!;
  }

  Schedule schedule;
  NannyWeekday selectedWeekday = NannyWeekday.values[DateTime.now().weekday - 1];
  ...
  void wantSchedule() async {
    if(!selectAll && idRoads.isEmpty) {
      NannyDialogs.showMessageBox(context, "Ошибка!", "Выберите хотя бы один маршрут!");
      return;
    }
    ...
    bool success = await DioRequest.handleRequest(
      context, 
      NannyDriverApi.wantScheduleRequest(request)
    );

    if(!success) {
      if(context.mounted) NannyDialogs.showMessageBox(context, "Ошибка!", "Не удалось отправить запрос на подтверждение!");
      return;
    }
