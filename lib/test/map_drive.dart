      ...
      return Scaffold(
          body: FutureLoader(
              ...
              completeView: (context, data) {
                print('complete call ${data}');
                return MapViewer(
                    onPanelBuild: (sc) => scrController = sc,
                    body: GoogleMap(
                        mapType: MapType.normal,
                        initialCameraPosition:
                            CameraPosition(target: data, zoom: 15),
                        onTap: (pos) =>
                            NannyMapGlobals.mapTapController.add(pos),
                        polylines: NannyMapGlobals.routes.value,
                        markers: NannyMapGlobals.markers.value),
                    panel: Navigator(
                        initialRoute: '/', onGenerateRoute: onRouteGen));
              },
  ...
  Route? onRouteGen(RouteSettings settings) {
    if (settings.name == '/') {
      return MaterialPageRoute(
          builder: (context) => AddressChooseView(
              baseContext: context, scrController: scrController));
    }
    return null;
  }
}
