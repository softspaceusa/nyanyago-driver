...
import 'package:nanny_components/base_views/views/web_view_screen.dart';
...
class AddCardVM extends ViewModelBase {
  ...
  void tryPay() async {
    ...
    await _waitForSmhWebOperation(html: initRes.html);
  ...
  void trySbpPay() async {
    ...
    await _waitForSmhWebOperation(url: initRes.paymentUrl);
  ...
  Future<void> _waitForSmhWebOperation({String? url, String? html}) async {
    ...
    if (html != null) {
      final file = await _saveHtmlToFile(html);
      await Navigator.of(context).push(
        MaterialPageRoute(
          builder: (context) => WebViewScreen(filePath: file.path),
        ),
      );
    } else if (url != null) {
  ...
  Future<File> _saveHtmlToFile(String html) async {
    final directory = await getTemporaryDirectory();
    final file = File('${directory.path}/temp_page.html');
